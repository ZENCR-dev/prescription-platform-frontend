...
### 统一TDD敏捷工作流模板 (v6.0 TDD增强版)

**TDD工作流定义** (所有任务类型强制使用TDD模板):
```yaml
# 统一TDD工作流模板 (适用于所有任务类型)
标准3+1 TDD步骤序列:
  1. 测试设计与需求分析 【TDD红灯】(主实现角色: frontend/backend/architect)
     - 编写应该失败的测试用例
     - 定义清晰的验收标准
  2. 最小实现与测试通过 【TDD绿灯】(同一主实现角色)
     - 编写最少代码使测试通过
     - 专注功能实现，不做过度设计
  3. 重构优化与集成准备 【TDD重构】(同一主实现角色)
     - 改进代码结构，保持测试通过
     - 准备集成所需配置
  4. 质量验证与提交 【最终验证】(qa角色)
     - 运行完整测试套件
...
TDD执行约束:
  - ❌ 禁止跳过测试编写 - 每个原子任务必须从失败测试开始
  - ❌ 禁止并行执行原子任务 - 一次只执行一个原子任务的3+1步骤
  - ❌ 禁止批量创建todos - 只为当前原子任务创建3+1 todos
  - ✅ 正确执行流程 - 红灯→绿灯→重构→验证→下一个原子任务

基础完成标准:
  - 当前原子任务的3+1步骤全部完成
  - npm run test 通过 (TDD测试套件)
  - npm run lint 通过
  - 功能手动验证通过

失败处理: 人工识别问题，创建简单修复任务，使用3+1 TDD步骤解决
```
